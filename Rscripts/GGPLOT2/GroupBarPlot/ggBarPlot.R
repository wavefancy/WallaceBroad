...
Usage:
    ggGroupBarPlot.R -x xname -y yname -o <filename> -W float -H float [--ymin iname --ymax aname] [--ylim nums]
...
if(!is.na(ymin)){
  ...
  p = p + geom_errorbar(aes_string(ymin = ymin, ymax = ymax), position = position_dodge(0.8), width = 0.2)
}
