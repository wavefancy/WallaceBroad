...
Notes:
  ...
  * For the dependent variable, regress out the covariates 
  *     and then scale back by add mean to the regression residual.

Usage:
  GLMCovariateStandardizer.R -c file -f formula -i id [--dc int] [--gf family]
...
yname      = iformula %>% stri_split_regex(.,pattern='[~]')   %>% unlist %>% str_trim %>% head(.,n=1)
idname     = opts$i
