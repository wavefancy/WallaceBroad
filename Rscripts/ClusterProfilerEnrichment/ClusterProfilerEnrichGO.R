...
Usage:
    ClusterProfilerEnrichGO.R -t file -o text [-b file] [-p value] [-q value]
    ClusterProfilerEnrichGO.R -h [--help]
...
for(k in c('CC', 'BP', 'MF')){
    ego <- enrichGO(gene      = as.character(topG[,1]),
                ...
                qvalueCutoff  = qcut,
                readable      = TRUE)
    export(as.data.frame(ego), file=paste0(outF,'_GO_',k,'.tsv.gz'))
}
